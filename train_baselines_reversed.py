...
def eval_with_postclick(model, sess, postclick_file, max_time_len, reg_lambda):
    preds = []
    labels = []
    losses = []
    user_ids = []  # 用于存储用户ID

    # 使用 DataLoader_Postclick 加载后点击行为数据
    data_loader = DataLoader_Postclick(EVAL_BATCH_SIZE, postclick_file, max_time_len)
    
    t = time.time()
    for user_seq_batch, user_seq_len_batch in data_loader:
        # 假设模型有一个方法可以处理 post-click 数据进行评估
        pred, label, loss = model.eval_with_postclick(sess, user_seq_batch, user_seq_len_batch, reg_lambda)
        preds += pred
        labels += label
        user_ids += [user_id[0] for user_id in user_seq_batch]  # 假设 user_seq_batch 中包含用户ID信息
        losses.append(loss)
...
def train(data_set_name, target_train_file, target_vali_file, user_seq_file, user_feat_dict_file, item_feat_dict_file, 
        ...
        user_fnum, item_fnum, emb_initializer, user_seq_postclick_file):
    tf.reset_default_graph()
    ...
    if model_type == 'GRU4Rec':
        ...
        item_emb = train_din_and_get_item_embeddings(dataset_size, feature_size, eb_dim, hidden_size, max_time_len, user_fnum, item_fnum, emb_initializer, lr, reg_lambda, train_batch_size, user_seq_file, target_train_file, user_feat_dict_file, item_feat_dict_file)
        model = TWIN_V2(feature_size, eb_dim, hidden_size, max_time_len, user_fnum, item_fnum, emb_initializer, item_embeddings=item_emb, n_clusters=100)
    elif model_type == 'ETA':
        model = ETA(feature_size, eb_dim, hidden_size, max_time_len, user_fnum, item_fnum, emb_initializer)
    elif model_type == 'SDIM':
        model = SDIM(feature_size, eb_dim, hidden_size, max_time_len, user_fnum, item_fnum, emb_initializer)
        ...
        model = DGIN(feature_size, eb_dim, hidden_size, max_time_len, user_fnum, item_fnum, emb_initializer)
    elif model_type == 'DGIN_V2':
        item_emb = train_din_and_get_item_embeddings(dataset_size, feature_size, eb_dim, hidden_size, max_time_len, user_fnum, item_fnum, emb_initializer, lr, reg_lambda, train_batch_size, user_seq_file, target_train_file, user_feat_dict_file, item_feat_dict_file)
        model = DGIN_V2(feature_size, eb_dim, hidden_size, max_time_len, user_fnum, item_fnum, emb_initializer, item_embeddings=item_emb, n_clusters=100)
    else:
        print('WRONG MODEL TYPE')
        exit(1)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if model_type != "DGIN" and model_type != "DGIN_V2":
            ...
            postclick_loader = DataLoader_Postclick(batch_size=train_batch_size, seq_postclick_file=user_seq_postclick_file, max_len=max_time_len)
            for epoch in range(1):
                if early_stop:
                    break
                ...
                for user_seq_batch, user_seq_len_batch in postclick_loader:
                    ...
                    if step % eval_iter_num == 0:
                        ...
                        vali_loss, logloss, rig, auc, gauc = eval_with_postclick(model, sess, target_vali_file, user_seq_file, user_feat_dict_file, item_feat_dict_file, max_time_len, reg_lambda)
                        training_monitor['vali_loss'].append(vali_loss)
                        training_monitor['logloss'].append(logloss)
                        training_monitor['rig'].append(rig)
                        training_monitor['auc'].append(auc)
...
if __name__ == '__main__':
    ...
    if data_set_name == 'tmall':
        ...
        user_seq_postclick_file = DATA_DIR_TMALL + 'user_seq_postclick.txt'
        ...
        user_seq_postclick_file = DATA_DIR_TAOBAO + 'user_seq_postclick.txt'
        ...
        user_seq_postclick_file = DATA_DIR_ALIPAY + 'user_seq_postclick.txt'
